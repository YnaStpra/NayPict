...
const insightsService = {
  ...
  async getOverview(): Promise<InsightsOverviewVo> {
    ...
    // Calculate start of today (UTC or local)
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString();
    
    // Calculate 7 days ago start
    const weekStart = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString();
    
    // Calculate 30 days ago start
    ...
    const totalFavorites = favoriteCountRes?.count ?? 0;
  ...
  async getViewsChart(range: '7d' | '30d' | '90d' = '7d', photoId?: string): Promise<InsightsChartDataVo> {
    ...
    // Query aggregated views grouped by date
    const rows = await orm
      .select({
        day: sql<string>`TO_CHAR(DATE(${photoViewTab.viewedAt}), 'YYYY-MM-DD')`,
        views: count(),
      })
      .from(photoViewTab)
      .where(and(...conditions))
      .groupBy(sql`DATE(${photoViewTab.viewedAt})`)
      .orderBy(sql`DATE(${photoViewTab.viewedAt}) ASC`);
