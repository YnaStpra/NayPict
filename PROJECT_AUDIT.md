...
| 🟢 **RESOLVED** | Community Engagement & Reaction Analytics | `photo_reaction`, `reaction-service.ts`, `insights-service.ts`, `admin/insights/page.tsx`, `photo-insights-dialog.tsx` | Visitor micro-reactions (❤️, 🔥, 📸, 📍, 👏) with Cookie-First zero-collision tracking, official photographer badges/pins/hearts, and integrated admin reaction analytics leaderboard & reset control in Photo Insights. |
...
16. **Zero-Login Instant Micro-Reactions & 1-Like Fingerprinting**: Pengunjung umum dapat berekspresi secara instan tanpa perlu mendaftar/login. Sistem menerapkan aturan eksklusif di mana 1 pengunjung hanya boleh memilih maksimal 1 reaksi emoji (❤️ *Love*, 🔥 *Incredible*, 📸 *Great Shot*, 📍 *Want to Visit*) dan 1 Like terpisah per foto dengan animasi partikel meletup (*burst particles*). Dilengkapi proteksi anti-spam ganda (*Hybrid Fingerprinting*) berbasis gabungan Cookie 1 tahun `naypict_vid` + Hash SHA-256 (IP Address + Browser User-Agent) untuk mencegah manipulasi via Incognito mode.
